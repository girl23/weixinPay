...
{
    //初始构造函数
    //payUrl接口商户系统先调用该接口在微信支付服务后台生成预支付交易单，返回正确的预支付交易回话标识后再在APP里面调起支付。
...
//   上面过程 (生成预支付订单XML上传到微信的预支付接口，预支付接口返回XML相应的的信息（prepayid等）并对签名进行验证)
